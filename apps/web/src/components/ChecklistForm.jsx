...
export default function ChecklistForm({
  ...
  return (
    <div className="wiz-card rise" style={{ position: 'relative' }}>
      {loading && (
      ...
      <button type="button" className="back-btn" onClick={onBack} disabled={loading}>
        ← {CATEGORY_META[category]?.label} 내용 입력
      </button>

      <h2 className="step-title">위험 신호를 확인하세요</h2>

      {suggestedItems.length > 0 ? (
        <div className="ai-suggest-notice">
          <span className="ai-notice-badge">AI 감지 완료</span>
        ...
        <div
          className="ai-suggest-notice"
          style={{ background: 'rgba(156,163,175,0.06)', borderColor: 'rgba(156,163,175,0.2)' }}
        >
          <span
            className="ai-notice-badge"
            style={{ background: 'rgba(156,163,175,0.15)', color: 'var(--text-3)' }}
          >
            AI 미감지
          </span>
          <span className="ai-notice-text">
            입력 텍스트에서 자동 감지된 신호가 없어요. 직접 해당하는 항목을 선택하세요.
          </span>
        </div>
      )}
